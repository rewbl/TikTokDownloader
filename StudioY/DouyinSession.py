...
class DouyinSession:
    ...
    def load_session(self):
        ...
        self.__parse_cookie_to_jar(self.cookie_jar, self.cookie)

    @property
    def cookie_header_value(self):
        return self.__cookie_jar_to_header(self.cookie_jar)
    ...
    def __parse_cookie_to_jar(self, cookie_jar, cookie_header):
        for key, value in self.__cookie_dict(cookie_header).items():
            key = unquote(key)
            value = unquote(value)
            cookie = Cookie(
                version=0,
    ...
    def __cookie_jar_to_header(self, cookie_jar: RequestsCookieJar) -> str:
        cookie_list = []
