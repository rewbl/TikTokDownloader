...
def _parse_cookie_to_jar(cookie_jar, cookie_header):
    for key, value in _cookie_dict(cookie_header).items():
        key = unquote(key)
        value = unquote(value)
        cookie = Cookie(
            version=0,
...
def _cookie_jar_to_header(cookie_jar: RequestsCookieJar) -> str:
    cookie_list = []
...
class DouyinSession:
    ...
    def load_session(self):
        ...
        cookie_dict = _cookie_dict(self.cookie)
        self.cookie = _cookie_dict_to_str(cookie_dict)
        _parse_cookie_to_jar(self.cookie_jar, self.cookie)

    @property
    def cookie_header_value(self):
        return _cookie_jar_to_header(self.cookie_jar)
